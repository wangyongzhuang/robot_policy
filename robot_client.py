...
def draw_robot_right(screen, x, y, color=RED):
    rect = [x - robot_size[0]/4, y - robot_size[1]/4, robot_size[0]/2, robot_size[1]/2]
    pygame.draw.rect(screen, color, rect, 0)
...
def visible(pos, target, map_img):
    dx = int(robot_size[0]/2)
    dy = int(robot_size[0]/2)
    targets = [[target[0] + dx, target[1] + dy], [target[0] + dx, target[1] - dy], [target[0] - dx, target[1] + dy],
               [target[0] - dx, target[1] - dy]]
    num = 0
...
def draw_state(screen, bars, state_1, state_2, info_1, info_2):
    ...
    draw_state_right(screen, state_1, state_2)
...
def draw_info(screen, bars, info_1, info_2):
    ...
    draw_info_right(screen,info_1, info_2)

    pygame.display.update()

def draw_info_right(screen, info_1, info_2):
    # top_right
    draw_robot_right(screen, info_1[0][0]/2 + 8000*scale, info_1[0][1]/2, RED)
    draw_robot_right(screen, info_1[1][0]/2 + 8000*scale, info_1[1][1]/2, RED)
    if visible([info_1[0][0], info_1[0][1]], [info_2[0][0], info_2[0][1]], raw_map_img)\
        ...
        draw_robot_right(screen, int(info_2[0][0] / 2) + 8000 * scale, int(info_2[0][1] / 2), GREEN)
    if visible([info_1[0][0], info_1[0][1]], [info_2[1][0], info_2[1][1]], raw_map_img)\
        ...
        print("red see green1")
        draw_robot_right(screen, int(info_2[1][0] / 2) + 8000 * scale, int(info_2[1][1] / 2), GREEN)

    # bottom right
    draw_robot_right(screen, int(info_2[0][0] / 2) + 8000 * scale, int(info_2[0][1] / 2) + 2500*scale, GREEN)
    draw_robot_right(screen, int(info_2[1][0] / 2) + 8000 * scale, int(info_2[1][1] / 2) + 2500*scale, GREEN)
    if visible([info_2[0][0], info_2[0][1]], [info_1[0][0], info_1[0][1]], raw_map_img)\
        ...
        draw_robot_right(screen, int(info_1[0][0] / 2) + 8000 * scale, int(info_1[0][1] / 2), GREEN)
    if visible([info_2[0][0], info_2[0][1]], [info_1[1][0], info_1[1][1]], raw_map_img)\
        ...
        print("red see green1")
        draw_robot_right(screen, int(info_1[1][0] / 2) + 8000 * scale, int(info_1[1][1] / 2), GREEN)

def draw_state_right(screen, state_1, state_2):
    # top right
    draw_robot_right(screen, int(state_1[0]['x'] / 2) + 8000 * scale, int(state_1[0]['y'] / 2), RED)
    draw_robot_right(screen, int(state_1[1]['x'] / 2) + 8000 * scale, int(state_1[1]['y'] / 2), RED)
    if visible([state_1[0]['x'], state_1[0]['y']], [state_2[0]['x'], state_2[0]['y']], raw_map_img)\
        ...
        print("red see green1")
        draw_robot_right(screen, int(state_2[0]['x'] / 2) + 8000 * scale, int(state_2[0]['y'] / 2), GREEN)
    if visible([state_1[0]['x'], state_1[0]['y']], [state_2[1]['x'], state_2[1]['y']], raw_map_img)\
        ...
        draw_robot_right(screen, int(state_2[1]['x'] / 2) + 8000 * scale, int(state_2[1]['y'] / 2), GREEN)

    # bottom right
    draw_robot_right(screen, int(state_2[0]['x'] / 2) + 8000 * scale, int(state_2[0]['y'] / 2) + 2500*scale, GREEN)
    draw_robot_right(screen, int(state_2[1]['x'] / 2) + 8000 * scale, int(state_2[1]['y'] / 2) + 2500*scale, GREEN)
    if visible([state_2[0]['x'], state_2[0]['y']], [state_1[0]['x'], state_1[0]['y']], raw_map_img)\
        ...
        draw_robot_right(screen, int(state_1[0]['x'] / 2) + 8000 * scale, int(state_1[0]['y'] / 2) + 2500 * scale, RED)
        print("green see red1")
    if visible([state_2[0]['x'], state_2[0]['y']], [state_1[1]['x'], state_1[1]['y']], raw_map_img)\
        ...
        draw_robot_right(screen, int(state_1[1]['x'] / 2) + 8000 * scale, int(state_1[1]['y'] / 2) + 2500 * scale, RED)
