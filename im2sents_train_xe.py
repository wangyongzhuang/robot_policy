...
loss = loss_ori
tf.summary.scalar('loss_ori', flag.loss_scale * loss_ori)
...
train_writer = tf.summary.FileWriter(flag.sum_dir+'_xe', sess.graph)

saver = tf.train.Saver(max_to_keep=100)
ckpt  = tf.train.get_checkpoint_state(flag.ckpt_dir+'_xe')
if flag.is_finetune:
    saver.restore(sess, ckpt.all_model_checkpoint_paths[-1])
else:
    fs = os.listdir(flag.ckpt_dir)
    for f in fs:
        os.remove(flag.ckpt_dir + '/' + f)
...
while early_stop < 5/flag.epoch_val:
    ...
    batch_feat_tmp, word_inds_tmp, word_refs_tmp = sess.run([data.samp_train_feat, data.samp_train_inds, data.samp_train_refs])
    scores_sub, scores_scst, scores_max = train_test_final(model, data, flag, sess, batch_feat_tmp, word_inds_tmp, word_refs_tmp, fr, score_type='SUM')
    
    # optimize
    _, s, l_rl, l, r_0 = sess.run([optimizer, merged, loss, loss_ori, recall_rate_0], feed_dict={learning_rate:[lr], reward_sents:scores_sub, model.batch_feat_train:batch_feat_tmp,   model.word_inds_train:word_inds_tmp, model.refs_train:word_refs_tmp, model.fire_rate:[fr]})
    ...
    # show the result
    if global_step% 10 == 0:
        print 'Epo_%0.2f_Itr_%d loss:%0.3f l_mean:%0.3f l_rl:%0.3f s_max:%0.3f s_scst:%0.3f recall:%0.3f'%(epoch, global_step, flag.loss_scale * l, flag.loss_scale * l_pre, flag.loss_scale * l_rl_pre, s_max_pre, s_scst_pre, r_0)

    # val and save model
    if global_step% int(flag.epoch_val * flag.data_size / flag.batch_size) == 0:
        # val
        #score = test_final(model, data, flag, sess)
        score = test_final(model, data, flag, sess, batch_feat_tmp, word_inds_tmp, word_refs_tmp)
